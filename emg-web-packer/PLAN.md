...
**状態: 実装済み。使い方と現在の設計は `README.md` を参照。**
このファイルは着手前の計画と、実装中に判明して計画を修正した点の記録として残している。
最終更新: 2026-08-20
...
| 計画時の記述 | 実際 |
|---|---|
| アトラス上限超過時「レイヤーがスキップされる可能性」 | `TexturePacker` は**例外を投げる**（`Exceeded max atlas size 8192`）。黙って欠けることはない |
| `.kra` はスコープ外 | `FileLoader` が拡張子で振り分け済みで、**追加コストなしで対応できた** |
| ベース選択の単位は「ルート直下の項目」 | `partID` はネストしたグループの内側の名前で上書きされるため一致しない。**実際の `partID` 単位**にした |
| （記載なし） | **非表示レイヤーの扱いが最重要だった。** PSD の差分は1枚だけ表示され残りは非表示なので、可視レイヤーだけ書き出すと `switch` パーツの差分が全滅する（実測で 17 → 3 レイヤーに欠落）。`switch` は全レイヤー、`static` は可視のみ、という扱いに変更した |
